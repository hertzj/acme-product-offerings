...
window.addEventListener('hashchange', ev => {
    if (window.location.hash.slice(1)) { // all prodcuts
        const products = [...document.querySelectorAll('.product')];
        products.forEach(product => {
            if (product.dataset.prodId !== window.location.hash.slice(1)) {
                product.classList.add('hidden')
            }
            else {
                product.classList.remove('product')
                product.classList.add('productHighlight')
                const highlightedLink = document.querySelector('.productHighlight a');
                highlightedLink.setAttribute('href', '#')
            }
        })
    }
    else { // highlighted product
        console.log('hi')
        const hidden = [...document.querySelectorAll('.hidden')];
        hidden.forEach(hid => {
            hid.classList.remove('hidden');
        })
        const highlighted = document.querySelector('.productHighlight');
        const highlightedLink = document.querySelector('.productHighlight a');
        const newLink = highlighted.dataset.prodId;
        highlighted.classList.add('product')
        highlighted.classList.remove('productHighlight')

        highlightedLink.setAttribute('href', `#${newLink}`)
    }
})
